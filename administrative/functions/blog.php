...
function add()
{
	...
	if(!empty($_FILES['image']['name']))
	{
		...
		move_uploaded_file($imagetemp,"../../images/blog/".$image);
		$thumbimg = "../../images/blog/".$image;
        $thumbcrop = thumbnail_box("../../images/blog/thumb/$image",$thumbimg,408,306);
        $largecrop = thumbnail_box("../../images/blog/$image",$thumbimg,1000,400);
	}
	//
	$query = query("INSERT INTO `blog`(blog_category_id,`title`,image,description,sort, `status`) VALUES ('$blog_category_id','$title','$image','$description','$sort','$status')");
	if($query==1)
	{	
		redirect('../blog.php?msg=done');
	}
	else
	{
		redirect('../blog.php?error=1');
	}
}
//
function edit()
{
	$blog_id = $_POST["id"];
	$query = query("SELECT  `title`,`image`, `description`, sort,status FROM `blog` where blog_id='$blog_id'");
	$row = $query->row;
	$title= $row["title"];
	$image= $row["image"];
	...
	$status= $row["status"];
	?>
	<form role="form" class="form-horizontal" method="post" action="functions/blog.php?do=update" enctype="multipart/form-data">
...
function update()
{
	...
	if(!empty($_FILES['image']['name']))
	{
		...
		move_uploaded_file($imagetemp,"../../images/blog/".$image);
		$thumbimg = "../../images/blog/".$image;
        $thumbcrop = thumbnail_box("../../images/blog/thumb/$image",$thumbimg,400,300);
        $largecrop = thumbnail_box("../../images/blog/$image",$thumbimg,1000,400);
	}
	else
	{
	...
	//
	$query = query("update `blog` set blog_category_id='0',`title`='$title',image='$image',description='$blog',sort=$sort,`status`='$status' where blog_id='$blog_id'");
	if($query==1)
	{	
		redirect('../blog.php?msg=done');
	}
...
{
	$blog_id = $_GET["id"];
	$query = query("DELETE FROM `blog` WHERE blog_id='$blog_id'");
	if($query==1)
	{
		redirect('../blog.php?msg=done');
	}
